...
function isMedical(item) {
  return item.types.includes('meds') || item.types.includes('injectors')
}
...
async function fetchCatalog() {
  ...
  console.log(`Static Tarkov ${GAME_MODE} snapshot loaded: ${rawItems.length} total items`)
  ...
  return rawItems
    ...
    .map((item) => {
      ...
      return {
        ...
        category: isMedical(item) ? 'medical' : 'food',
        types,
...
const foodCountBeforeImages = selected.filter((item) => item.category === 'food').length
const medicalCountBeforeImages = selected.filter((item) => item.category === 'medical').length

if (selected.length < 30 || foodCountBeforeImages === 0 || medicalCountBeforeImages === 0) {
  throw new Error(
    `Suspicious allergy catalog: ${selected.length} total, ${foodCountBeforeImages} food, ${medicalCountBeforeImages} medical`,
  )
}
...
const output = `import type { TarkovItem } from '../types'\n\n` +
  `// Generated by scripts/sync-items.mjs from json.tarkov.dev (${GAME_MODE}). Do not edit by hand.\n` +
...
console.log(`Catalog synced: ${items.length} items (${foodCountBeforeImages} food, ${medicalCountBeforeImages} medical/injectors)`)
